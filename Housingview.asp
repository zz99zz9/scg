...
%>
       ...
       <%if rrs("jgjj")>1 then%> <div class="col-md-5 r-line col-xs-6"><span class="fc-red">总价$<%if rrs("jgjj")<1 then%>0<%end if%><%=rrs("jgjj")%>万起</span><em style="color:#bf0000;">约<%=rrs("jgzj")%>万人民币<i class="fz13p fc-gray">(首付10%)</i></em></div><%end if%><div class="col-md-7 pl20 col-xs-6"><%=rrs("mj1")%>～<%=rrs("mj2")%><span class="fz15">㎡ 套内面积</span><em>居室：<%=rrs("hx")%></em></div>
